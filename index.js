//Variables
var autoroles = ['she / her', 'he / him', 'they / them', 'it / that', 'hide spanish', 'hide english', 'hide portuguese', 'hide french', 'showdown!', 'cq-80']
...
client.on('userUpdate', (oldUser, newUser) => {
  ...
  if (oldUser.avatarURL() !== newUser.avatarURL() && userRoles.cache.find(role => role.id === "584594259550797824")) {
    ...
    collector.on('collect', cMessage => {
      ...
      if (numb) {userRoles.color.setColor(colors[--numb].toString())}//Variables
...
client.on('message', message => {
  ...
  //Rol
  else if (msgCon.startsWith(prefix + 'role ')) {
    var roles = message.member.roles
    if (roles.color.id === "563155114886561792") {
      message.guild.roles.create({data: {name: argresult, color: "WHITE", position: 16, permissions: 0}}).then(role => {
      roles.add(role.id)})}
